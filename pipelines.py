...
class QGPipeline:
    ...
    def __call__(self, inputs: str, card_style: FlashcardStyle):
        ...
        # if card_style == FlashcardStyle.QA:
        #     qg_examples = self._prepare_inputs_for_qg_from_answers_prepend(inputs, answers)
        # else:
        qg_examples = self._prepare_inputs_for_qg_from_answers_hl(sents, answers)

        qg_inputs = [example['source_text'] for example in qg_examples]
        if card_style == FlashcardStyle.Cloze:
            return [cloze[19:] for cloze in qg_inputs]
...
def qg_pipeline(
    ...
    **kwargs,
    ...
    if task == "e2e-qg":
        ...
        return task_class(model=model, tokenizer=tokenizer, ans_model=ans_model, ans_tokenizer=ans_tokenizer, use_cuda=use_cuda)
    else:
        return task_class(model=model, tokenizer=tokenizer, ans_model=model, ans_tokenizer=tokenizer, use_cuda=use_cuda)
